...
#include "roq/cme/protocol/mdp/md_instrument_definition.hpp"
...
ReferenceData create_reference_data(::cme::sbe::mdp::MDInstrumentDefinitionFuture54 const &message, uint16_t stream_id, tools::Security &security) {
  ...
  auto quote_currency = protocol::mdp::get_string_view(value.currency(), value.currencyLength());
  auto settlement_currency = protocol::mdp::get_string_view(value.settlCurrency(), value.settlCurrencyLength());
  auto min_price_increment = map(value.minPriceIncrement()).template get<double>();
  auto contract_multiplier = protocol::mdp::get_int(value.contractMultiplier(), value.contractMultiplierNullValue());
  ...
  return {
      ...
      .external_security_id = value.securityID(),
      .cfi_code = {},
      .base_currency = {},
...
ReferenceData create_reference_data(::cme::sbe::mdp::MDInstrumentDefinitionOption55 const &message, uint16_t stream_id, tools::Security &security) {
  ...
  auto min_trade_vol = utils::safe_cast(value.minTradeVol());
  auto max_trade_vol = utils::safe_cast(value.maxTradeVol());
  auto strike_currency = protocol::mdp::get_string_view(value.strikeCurrency(), value.strikeCurrencyLength());
  return {
      .stream_id = stream_id,
      .exchange = security.exchange,
      .symbol = security.symbol,
