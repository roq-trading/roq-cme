...
TEST_CASE("simple", "[mbofd_recovery]") {
  ...
  struct MyHandler final : public sbe::Parser::Handler {
    ...
    void operator()(Trace<cme_mdp3::SnapshotFullRefresh52> const &event, sbe::Frame const &) override {
      ...
      switch (++count_52) {
        case 1: {
          ...
          CHECK(sbe::get_double(value.lowLimitPrice()) == 0.00390625_a);
          CHECK(sbe::get_double(value.maxPriceVariation()) == 0.125_a);
          value.sbeRewind();  // wtf!
        ...
        case 2: {
          ...
          CHECK(value.mDSecurityTradingStatus() == cme_mdp3::SecurityTradingStatus::ReadyToTrade);
          CHECK(std::isnan(sbe::get_double(value.highLimitPrice())));
          CHECK(sbe::get_double(value.lowLimitPrice()) == 0.03125_a);
    ...
    void operator()(Trace<cme_mdp3::SnapshotFullRefreshOrderBook53> const &event, sbe::Frame const &) override {
      using value_type = std::remove_cvref<decltype(event)>::type::value_type;
      auto &value = const_cast<value_type &>(event.value);  // note! not const-safe
      switch (++count_53) {
        case 1: {
